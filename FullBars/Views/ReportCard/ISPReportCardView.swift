...
/// Shareable ISP comparison card — shows what you pay for vs. what you get.
...
struct ISPReportCardView: View {
    ...
    private var deliveryPercent: Int {
        guard profile.ispPromisedSpeed > 0 else { return 0 }
        return Int((avgDownload / profile.ispPromisedSpeed) * 100)
    }

    private var deliveryGrade: String {
        switch deliveryPercent {
        case 90...:  return "A"
        case 75..<90: return "B"
        case 60..<75: return "C"
        case 40..<60: return "D"
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 8) {
                HStack {
                    Image(systemName: "building.2.fill")
                        .foregroundStyle(primary)
                    Text(profile.ispName.isEmpty ? "Your ISP" : profile.ispName)
            ...
            VStack(spacing: 8) {
                ZStack {
                    ...
                    Circle()
                        .trim(from: 0, to: min(1.0, Double(deliveryPercent) / 100.0))
                        .stroke(deliveryColor, style: StrokeStyle(lineWidth: 8, lineCap: .round))
                        .frame(width: 100, height: 100)
            ...
            VStack(spacing: 14) {
                ...
                comparisonRow(
                    label: "Upload Speed",
                    promised: "—",
                    actual: "\(Int(avgUpload)) Mbps",
                    isGood: true
                )
                comparisonRow(
                    label: "Latency",
                    promised: "< 30 ms",
                    actual: "\(Int(avgLatency)) ms",
    ...
    private func comparisonRow(label: String, promised: String, actual: String, isGood: Bool) -> some View {
        HStack {
            Text(label)
                .font(.system(.caption, design: .rounded))
                .foregroundStyle(.white.opacity(0.6))
